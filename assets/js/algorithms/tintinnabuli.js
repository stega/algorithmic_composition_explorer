...
function generateNotes(e) {
  ...
  var chordTones = chordToneSelect.options[chordToneSelect.selectedIndex].text.split('-')
...
function convertPatternToNoteLengths(pattern) {
  ...
  for(const [index,trig] of pattern.entries()){
    if(trig == 1){ // if the step is active
      var stepLength = 1
      ...
      if(stepLength == 8){
        ...
        if( !Number.isInteger(8 / stepLength) ){ dot = '.' }
...
function seqPlus(e){
  ...
  if(numElems < 16){ // don't create more than 8 steps
    var elem = "<input type=\"checkbox\" class=\"btn-check\" id=\"seqxxx\" autocomplete=\"off\"><label class=\"btn btn-outline-primary me-1\" for=\"seqxxx\">xxx</label>"
...
function seqMinus(e){
  ...
  if(seq.length > 1){ // don't remove the last step!
